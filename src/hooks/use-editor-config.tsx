...
import { DUAL_VIEW_MODES, EDualVIewMode, MODE_TO_TITLE } from "@/types/episode-type";
import { useSession } from "next-auth/react";
import { useCallback, useEffect, useMemo, useRef } from "react";
import { toast } from "sonner";
import { ContentDisplay, EEditorKit, EPlatform, ESidebar, RteData, ToolbarTypes, TRteContent, TRteDualViewRenderData } from "unified-editor";
...
export default function useEditorConfig() {
	...
	const { data: contentData, isPending: isContentPending, latestStatus } = useEpisodeContent()
									...
									<Button
										...
										onClick={() => {
											void navigator.clipboard.writeText(
												getTextFromTextOrValue(contentData?.additional_view?.[k] || '')
											)
	...
	const modeToComponent: Record<string, TRteDualViewRenderData> = useMemo(
		() => ({
			...
			[EDualVIewMode.BASE_SCRIPT]: {
				fetchContent: async () => (await getEpisodeContent(Number(contentData?.chapter?.parent || contentData?.chapter?.id)))?.text || ""
			},
			[EDualVIewMode.PREV_EP]: { fetchContent: () => getPrevEpContent(contentData) },
			[EDualVIewMode.NEXT_EP]: { fetchContent: () => getNextEpContent(contentData) },
	...
	const editorConfig = useMemo(() => {
		const data: RteData = {
			...
			sidebarConfig: {
				...
				enabledDefaultSidebars: [ESidebar.CHATBOT, ESidebar.COMMENTS, ESidebar.DUAL_VIEW, ESidebar.FAR, ESidebar.NOTES, ESidebar.OUTLINE, ESidebar.BEAT_SHEET],
				sidebarButtons: [ESidebar.CHATBOT, ESidebar.OUTLINE],
				defaultSidebarConfig: {
					[ESidebar.CHATBOT]: {
						quickPrompts: contentData?.chapter?.language === ELanguage.GERMAN_ORIGINAL ? QUICK_PROMPTS : QUICK_PROMPTS_EN,
					},
					[ESidebar.BEAT_SHEET]: {
						enableBeatSheetEditor: hasNWMRan(contentData?.chapter),
						getChapterCharacters: () => fetchChapterCharacters({ chapter_id: Number(contentData?.chapter.id || 0) }),
						getScenesMetadata: () => getScenesMetadata(contentData?.chapter.id)
					},
					[ESidebar.NOTES]: {
						getNotes: () => getNotes(Number(contentData?.chapter.project)),
						updateNotes: ({ params }) => updateNotes({ project_id: contentData?.chapter?.project || 0, params })
					},
					[ESidebar.FAR]: {
						downloadLOCSheetFunc: () => downloadLOCSheet(String(contentData?.chapter?.project || 0)),
						getLOCSheet: () => getLOCSheet(Number(contentData?.chapter?.project)),
						showAddForm: contentData?.chapter.language === ELanguage.GERMAN_ORIGINAL,
						showSuggestions: contentData?.chapter.language === ELanguage.GERMAN_ORIGINAL,
						// remaining
					},
					outline: {
			...
			extraConfig: {
				getMetadata: async ({ start, end }) => (await getMetadata(Number(contentData?.chapter?.project), start, end, contentData?.chapter?.language)).data,
				storyData: initialStoryData,
			},
			accessControlConfig: {
				disableEditing: !isWriter || latestStatus !== contentData?.chapter?.status,
				enableAccessControl: !isWriter || latestStatus !== contentData?.chapter?.status,
		...
		return data;
	}, [onContentChange, contentData, session, isContentPending, modeToComponent, modeToTitle, modes, initialStoryData, latestStatus]);
