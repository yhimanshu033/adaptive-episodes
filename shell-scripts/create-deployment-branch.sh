...
# Enhanced Release Script with Git Tagging and Changelog Generation
# 
# This script creates a release branch from staging with the following features:
# - Interactive version selection (major/minor/patch/no change)
# - Merges latest main changes into staging
# - Creates release branch with version update
# - Generates git tags for releases
# - Auto-generates changelog using conventional commits
# - Creates GitHub PR automatically
#
# Usage: npm run release (from staging branch)
# Dependencies: git, gh (optional), npx, conventional-changelog-cli
...
if [ "$new_version" != "$current_version" ]; then
    ...
    if command -v npx &> /dev/null; then
        ...
        if [ $? -eq 0 ]; then
            ...
            echo -e "${YELLOW}Warning: Could not generate changelog entry${NC}"
        fi
    else
        echo -e "${YELLOW}Warning: npx not found, skipping changelog generation${NC}"
    fi
    
    # Add updated files
    echo -e "${BLUE}Committing version update and changelog...${NC}"
    git add package.json
    # Add CHANGELOG.md if it exists and was modified
...
- Generated changelog entry for release
- Prepared for $version_type release" --no-verify
...
Branch: $branch_name
Base: staging with latest changes from main
...
if [[ $push_choice =~ ^[Yy]$ ]]; then
    ...
    # Push tags if version was updated
    if [ "$new_version" != "$current_version" ]; then
        echo -e "${BLUE}Pushing tags to remote...${NC}"
        git push origin --tags
        
        if [ $? -ne 0 ]; then
            echo -e "${YELLOW}Warning: Failed to push tags to remote${NC}"
            echo -e "${YELLOW}You may need to push tags manually: git push origin --tags${NC}"
        else
...
if [[ $push_choice =~ ^[Yy]$ ]]; then
    ...
    if [ "$new_version" != "$current_version" ]; then
        echo -e "  - Tags pushed: ${GREEN}Yes${NC}"
    fi
    if [[ $pr_choice =~ ^[Yy]$ ]]; then
    ...
    if [ "$new_version" != "$current_version" ]; then
        echo -e "  - Tags pushed: ${RED}No${NC}"
        echo -e "  - ${YELLOW}Remember to push tags: git push origin --tags${NC}"
    fi
fi