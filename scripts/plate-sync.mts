...
async function fetchPackageVersion(pkg: string) {
  try {
    ...
    console.error(
      `Error fetching version for ${pkg}:`,
  ...
  try {
    ...
    if (updatedPackages.length > 0) {
      ...
      try {
        ...
        console.error('Error running pnpm install:', (error as Error).message)
        process.exit(1)
      }
    } else {
    ...
    console.error('Error updating packages:', (error as Error).message)
    process.exit(1)
  }
}
